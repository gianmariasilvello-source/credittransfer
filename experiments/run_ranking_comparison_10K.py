...
def run_single_experiment_with_graph(graph, node_types, node_to_authors, metadata,
    ...
    avg_kudos = np.mean(list(h_indices_kudos.values())) if h_indices_kudos else 0.0
    avg_credit = np.mean(list(h_indices_credit.values())) if h_indices_credit else 0.0
    print(f"  ✓ H-index from kudos: max={max(h_indices_kudos.values()) if h_indices_kudos else 0}, avg={avg_kudos:.2f}")
...
def main():
    ...
    # Default config path (absolute)
    default_config = os.path.join(project_root, 'config', 'synthetic_10K.properties')

    if len(sys.argv) > 1:
        gen_config = sys.argv[1]
        # Make relative paths absolute
        if not os.path.isabs(gen_config):
            gen_config = os.path.join(project_root, gen_config)
    else:
    ...
    print("="*100)
    print(f"\nConfiguration file: {gen_config}")

    # Verify config file exists
    if not os.path.exists(gen_config):
        print(f"\n❌ ERROR: Config file not found: {gen_config}")
        print(f"\nSearching for config files in: {os.path.join(project_root, 'config')}")
        config_dir = os.path.join(project_root, 'config')
        if os.path.exists(config_dir):
            config_files = [f for f in os.listdir(config_dir) if f.endswith('.properties')]
    ...
    # Define retention strategies to compare (2 node types: papers, datasets)
    strategies = [
        {
            'name': 'Direct Citations Only (1.0, 1.0)',
            'rates': np.array([1.0, 1.0])
        },
        {
            'name': 'Medium Retention (0.7, 0.5)',
            'rates': np.array([0.7, 0.5])
        },
        {
            'name': 'Low Retention (0.5, 0.2)',
            'rates': np.array([0.5, 0.2])
        },
        {
            'name': 'Balanced Retention (0.5, 0.5)',
            'rates': np.array([0.5, 0.5])
        },
        {
            'name': 'Papers Only (0.5, 1.0)',
            'rates': np.array([0.5, 1.0])
        },
        {
            'name': 'Datasets Only (1.0, 0.5)',
